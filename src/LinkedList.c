...
struct Node*
addToList(List* list, int val, bool addToEnd)
{
	if (list->head == NULL)
...
bool
deleteAux(List* list, int val, struct Node* node, struct Node* prev)
{
	if (node == NULL)
		return false;
	if (node->val == val)
...
bool
del(List* list, int val)
{
	return deleteAux(list, val, list->head, NULL);
}

void
printAux(struct Node* node)
{
	if (node == NULL)
		return;
	printf("%d ", node->val);
